/**
 * Finds the articulation points (critical routers) in a graph.
 *
 * Articulation points are vertices in a graph whose removal would disconnect the graph. This function uses the Depth-First Search (DFS) algorithm to identify the articulation points in the given graph.
 *
 * @param adj The adjacency matrix representing the graph.
 * @param V The number of vertices (routers) in the graph.
 */
/**
 * Finds the articulation points (critical routers) in a graph.
 *
